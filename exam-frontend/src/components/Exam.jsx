...
const Exam = () => {
  ...
  const [code, setCode] = useState(''); // State to store the code written by the user
  const [executionResults, setExecutionResults] = useState(null); // State to store execution results
  ...
  const handleCompileAndRun = async () => {
    try {
      // Serialize test cases to ensure they are plain data
          ...
          <div className="mt-10 flex justify-between">
            ...
            {currentQuestionIndex < exam.questions.length - 1 && (
              <button
                ...
                className={`bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 ${currentQuestionIndex === 0 ? 'ml-auto' : ''}`}
              >
                Next
              </button>
            )}
